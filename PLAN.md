...
| **2** ✅ | Native PyCharm diff viewer, file/hunk navigation, hunk-to-terminal synchronization, and `CollaborationToolsHunkReviewUiHost` (`com.intellij.collaboration.ui`-based review chrome: comment bubbles, threaded replies, gutter affordances) for GH/GitLab-review visual parity | Native diff rendering, file/hunk selection, and the collaboration-tools UI host all compile against the real PyCharm 2026.2.2 SDK and pass `./gradlew build`; `./gradlew verifyPlugin` reports the plugin as **Compatible** with PY-262.10968.92 (internal/experimental API usage is expected — see open item 3, now resolved) |
...
| **4 (optional)** | VCS Log "Review commit/range with Hunk" ✅; TS push-extension replacing polling (not started) | Spike only, not required for a working MVP. `ReviewCommitWithHunkAction` added to `Vcs.Log.ContextMenu`: single selection → `hunk show <rev>`, multi-selection → `hunk diff <base> <head>` (oldest/newest by commit time via `VcsLogCommitSelection`, since VCS Log selection order is display order, not chronological). `./gradlew build` and `./gradlew verifyPlugin` pass, still **Compatible** with PY-262.10968.92 at the same experimental/internal/deprecated API counts as before (new code uses only stable `vcs.log`/`vcs.log.graph` APIs) |
...
3. ✅ Resolved: `com.intellij.collaboration.ui` classes used in Phase 2 do trip `verifyPlugin`'s internal/experimental-API detectors (`EditorComponentInlaysManager`, `RoundedPanel`, `LineGutterIconRenderer.getLine/setIconVisible/disposeInlay`, `AddCommentGutterIconRenderer`, `CodeReviewCommentUIUtil`, `CodeReviewChatItemUIUtil`/`.Builder` — 75 experimental + 6 internal + 6 deprecated + 28 scheduled-for-removal usages against PY-262.10968.92), but the verifier still reports the plugin **Compatible** — these are warnings, not build-breaking errors. This is the accepted "version-fragile, isolated here" trade-off from the architecture section; a future SDK break is confined to `ui/collab/CollaborationToolsHunkReviewUiHost.kt`.
